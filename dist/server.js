...
async function requestManager(req, res) {
    ...
    if (!hosts_1.default[req.headers.host]) {
        ...
        const html_404 = fs_1.default.readFileSync(path_1.default.join(__dirname, '../html/404.html'), 'utf-8');
        return res.end(html_404.replace(/{host}/g, req.headers.host));
    }
    ;
    // Get details about the registered target
    const { type, target, arc } = hosts_1.default[req.headers.host];
...
async function onError(err, req, res) {
    ...
    const html_500 = fs_1.default.readFileSync(path_1.default.join(__dirname, '../html/500.html'), 'utf-8');
