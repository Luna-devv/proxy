...
exports.onError = exports.requestManager = exports.proxy = void 0;
...
exports.proxy = http_proxy_1.default.createProxyServer({
    proxyTimeout: config_1.Config.outTimeout,
    timeout: config_1.Config.inTimeout,
    ws: true
}).listen(config_1.Config.port.proxy);
async function requestManager(req, res) {
    // Return error if host isn't congifured
    if (!hosts_1.default[req.headers.host]) {
        res.writeHead(404, {
            'Content-Type': 'text/html'
        });
        const html_404 = node_fs_1.default.readFileSync(node_path_1.default.join(__dirname, '../html/404.html'), 'utf-8');
        return res.end(html_404.replace(/{host}/g, req.headers.host));
    }
    ;
    // Get details about the registered target
    const { type, target, arc, ip, overwrites } = hosts_1.default[req.headers.host];
    ...
    switch (type) {
        ...
        case "WS":
            // WebSocket requests
            exports.proxy.ws(req, res.socket, {
...
async function onError(err, req, res) {
    ...
    try {
        ...
        const html_500 = node_fs_1.default.readFileSync(node_path_1.default.join(__dirname, '../html/500.html'), 'utf-8');
